...
// This module allows the rest of the project to hide the Binary to BCD, Seven Segment and the Rest
// Can treat the display as three independent decimal values after this module
...
module decimal_display_driver (
  ...
  // Generate loop repeats the hardware three times, once each for the first, second, and third pair\
  // blank0 blank1 blank2 will blank their respective seven-segment display pair
  genvar i;
  generate
    for (i = 0; i < 3; i = i + 1) begin : g_digit_pair
      ...
      binary_to_bcd u_binary_to_bcd (  // Converts value0 value1 value2 into tens and ones
          .bin(value_a[i]),  // value0 = value_a[0], value1 = value_a[1], value = value_a[2]
          .tens(tens),  // tens and ones are separated in binary_to_bcd module for each iteration
          .ones(ones)  // Remember that each of the three generate iterations cover display pairs
      );

      seven_segment #(
          .ACTIVE_LOW(1)
      ) u_ones_seven_segment (
          .digit(ones),
          .blank(blank_a[i]),  // Blanks the display
          .segments(hex_a[2*i])  // hex_a[2*i] is the ones digit HEX0 HEX2 HEX 4
      );

      seven_segment #(
          .ACTIVE_LOW(1)
      ) u_tens_seven_segment (
          .digit(tens),
          .blank(blank_a[i]),  // Blanks the display
          .segments(hex_a[2*i+1])  // hex_a[2*i + 1] is the tens digit HEX1 HEX3 HEX5
      );
